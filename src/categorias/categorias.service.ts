...
export class CategoriasService {
  ...
  async findAllAdmin(filter: CategoriaFilterDto) {
    ...
    const [categorias, totalItems] = await this.prisma.$transaction([
  ...
  async update(id: number, dto: PutCategoriaDto) {
    ...
    if (!dto.active) {
      return this.prisma.$transaction(async (transaction) => {
        const categoria = await transaction.categoria.update({
          where: {
            id,
          },
          data: {
  ...
  async delete(id: number): Promise<void> {
    ...
    if (categoria._count.produtos > 0) {
      await this.deactivateCategoriaAndProdutos(id);
  ...
  private async ensureCategoriaExists(id: number): Promise<void> {
    const categoria = await this.prisma.categoria.findUnique({
      where: {
        id,
      },
  ...
  private async deactivateCategoriaAndProdutos(id: number): Promise<void> {
